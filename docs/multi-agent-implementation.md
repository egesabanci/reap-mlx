...
- Codex CLI authenticated for the requested `gpt-5.5` model
...
AO notifications are disabled in `agent-orchestrator.yaml` so macOS `terminal-notifier` does not emit low-detail alerts. Use the dashboard and `ao status` for state.
...
The default worker is Codex with `gpt-5.5`. Codex reports `reasoning effort: xhigh` for this model in the local CLI. Use explicit overrides only for exceptional cases:
